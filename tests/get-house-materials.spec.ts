...
import {FileNotFoundError} from '../src/calculator/index'

describe("Loads a house object from a file as defined in IHouse", () => {
   it("should throw an Error due to no house named fjoiewa;", () => {
       expect(() => {getHouseMaterials("htgiaphhidfoigoajpgjerpiogfjvrepna")}).to.throw(FileNotFoundError, "There is no house by that name.");
   })
